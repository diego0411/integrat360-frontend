...
function ManageGroups() {
  ...
  const handleAddUserToGroup = async () => {
    if (!selectedGroup || !selectedUser) {
      return alert("❌ Selecciona un grupo y un usuario.");
    }

    try {
      const token = localStorage.getItem("token");
      await axios.post(`${import.meta.env.VITE_API_URL}/groups/${selectedGroup.id}/members`, {
        userId: selectedUser
      }, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleRemoveUserFromGroup = async (userId) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`${import.meta.env.VITE_API_URL}/groups/${selectedGroup.id}/members/${userId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  return (
    <Box className="container">
      ...
      {/* Lista de Grupos */}
      <Typography variant="h6" sx={{ mt: 3 }}>📋 Lista de Grupos</Typography>
      {groups.length === 0 ? (
        <Typography variant="body2">⚠️ No hay grupos disponibles.</Typography>
      ) : (
